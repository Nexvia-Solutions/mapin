...
/**
 * Regex-based directive scanner - SPEC.md section 4's documented `--no-boot` fallback (and,
 * since SPEC.md 1.21, also the structural half `BladeExtractor` composes internally when booted -
 * see that class for why structural edges stay regex-sourced rather than being reimplemented
 * against compiled output). Exact for the directives it targets (Blade directive syntax is fixed,
 * not something that varies by app): every edge comes from a literal string argument to a real
 * directive or tag, never a computed name.
 */
final class DirectiveScanner implements Extractor
{
    ...
    public function extract(SourceFile $file, ExtractionContext $ctx): Fragment
    {
        ...
        $nodes = [
            // Every .php file gets this from PhpExtractor::buildFragment() - a .blade.php never
            // did (SPEC.md 1.8 disclosed this once, unfixed), so the Declares edge two lines below
            // has always silently dropped in SqliteStore::insertEdges(): its `from` key never
            // resolved to a real node. Found again, and actually fixed, while giving
            // BladeExtractor's compiled-PHP edges a real file node of their own to land on
            // (SPEC.md 1.21) - without this they would have dropped exactly the same way.
            new Node(NodeType::File, $file->relativePath, $fileKey, $file->relativePath),
            new Node(NodeType::View, $viewName, $viewKey, $file->relativePath),
        ];
        $edges = [new Edge(EdgeType::Declares, $fileKey, $viewKey)];
